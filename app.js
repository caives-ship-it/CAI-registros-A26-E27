...
function initState(){
  ...
  // Prioridad: config guardada en este navegador (por si el staff la
  // cambió manualmente) > config incrustada en data.js (FIREBASE_CONFIG),
  // que es la que hace que TODOS los visitantes —incluidos los
  // estudiantes— se conecten automáticamente sin tener que hacer nada.
  const fbCfg = loadFbConfig() || (typeof FIREBASE_CONFIG !== "undefined" ? FIREBASE_CONFIG : null);
  ...
  wrap.innerHTML = activas.map(a => {
    ...
    <div class="catalog-item">
      <div class="tag-row">
        ...
        ${a.nivel ? `<span class="tag tag-nivel">${esc(a.nivel)}</span>` : ""}
        ${a.licenciatura ? `<span class="tag tag-carrera">${esc(a.licenciatura)}</span>` : ""}
      </div>
      <h3>${esc(a.nombre)}</h3>
      ${a.objetivo ? `<p>${esc(a.objetivo)}</p>` : ""}
...
function renderActividadesTab(){
  ...
  grid.innerHTML = STATE.actividades.map(a => {
    ...
    ` : `<p class="muted-sm" style="margin:8px 0 0;">Sin fechas programadas todavía.</p>`;
    ...
    <div class="actividad-card ${a.activa===false?"inactiva":""}">
      ...
      <h3>${esc(a.nombre)}</h3>
      ${a.licenciatura ? `<p class="meta">🎓 Dirigido a: ${esc(a.licenciatura)}</p>` : ""}
      <p class="meta">📍 ${esc(a.espacio)} · ⏱ ${esc(a.duracion)} · Cupo por sesión ${esc(String(a.cupo))}</p>
      ${a.objetivo ? `<p class="obj">${esc(a.objetivo)}</p>` : ""}
      ...
      <div class="sesiones-block">
        <div class="table-toolbar" style="margin-bottom:8px;">
          <span class="meta" style="font-weight:700; text-transform:uppercase; font-size:11px; letter-spacing:.04em;">Fechas programadas</span>
          <button class="btn-secondary" style="padding:6px 10px; font-size:12px;" onclick="newSesionForActividad('${a.id}')">+ Agregar otra fecha</button>
        </div>
        ${sesionesHtml}
      </div>
    </div>
...
function actividadFormHtml(a){
  a = a || { tipo:"Taller", nombre:"", nivel:"", duracion:"", espacio:"", cupo:10, activa:true };
  const esNueva = !a.id;
  return `
    ...
    ${esNueva ? `
    <div class="grid-2">
      <label>Fecha<input type="date" id="af_fecha" value="${todayISO()}" required></label>
      <label>Hora<input type="time" id="af_hora" value="12:00" required></label>
    </div>
    ` : ""}
    <label>Cupo${esNueva ? "" : " por sesión"}<input type="number" id="af_cupo" value="${a.cupo}" min="1"></label>
    <div class="btn-row">
      <button class="btn-secondary" id="afCancel">Cancelar</button>
      <button class="btn-primary" id="afSave">${esNueva ? "Guardar y programar" : "Guardar"}</button>
    </div>
  `;
}
...
function bindActividadForm(existing){
  ...
  document.getElementById("afSave").addEventListener("click", () => {
    ...
    const cupo = Math.max(1, parseInt(document.getElementById("af_cupo").value || "10", 10));
    const data = {
    ...
    if (existing){
      ...
      const hora = document.getElementById("af_hora").value;
      if (!fecha || !hora) return toast("Selecciona la fecha y hora de la primera sesión.", "error");
      const nueva = { id: uid("act"), activa:true, licenciatura:"", objetivo:"", ...data };
      STATE.actividades.push(nueva);
      STATE.sesiones.push({ id: uid("ses"), actividadId: nueva.id, fecha, hora, cupo, estado: "abierta" });
      persist();
      closeModal();
      toast("Actividad creada y programada", "ok");
    }
  });
}
